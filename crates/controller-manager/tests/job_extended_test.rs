...
fn make_job_with_success_policy(name: &str, namespace: &str, policy_type: &str) -> Job {
    ...
    if policy_type == "AtLeastOnce" {
        job.spec.success_policy = Some(SuccessPolicy {
            rules: Some(vec![SuccessPolicyRule {
...
fn make_job_with_pod_failure_policy(name: &str, namespace: &str, rule_type: &str) -> Job {
    ...
    if rule_type == "FailJob" {
        job.spec.pod_failure_policy = Some(PodFailurePolicy {
            rules: Some(vec![PodFailurePolicyRule {
                action: "FailJob".to_string(),
                on_exit_codes: Some(vec![PodFailurePolicyOnExitCodesRequirement {
                    container_name: "task".to_string(),
                    operator: "In".to_string(),
                    values: vec![1],
                }]),
...
async fn simulate_kubelet_cleanup(storage: &Arc<MemoryStorage>, namespace: &str) {
    let pods: Vec<Pod> = storage.list(&format!("/registry/pods/{}/", namespace))
        .await
        .unwrap();
...
async fn job_should_adopt_matching_orphan_pods() {
    ...
    assert_eq!(owner_refs.len(), 1, "Orphan pod should have one owner reference");
    assert_eq!(owner_refs[0].kind, "Job", "Owner should be a Job");
    assert_eq!(owner_refs[0].name, "adoption-test", "Owner should be our job");
    assert!(owner_refs[0].controller.unwrap(), "Owner should be controller");
}
...
async fn job_should_release_non_matching_pods() {
    ...
    job.spec.selector = LabelSelector {
...
async fn indexed_job_should_track_completion_per_index() {
    ...
    for (i, pod) in pods.iter().enumerate() {
        ...
        if let Some(ref mut annotations) = pod_with_index.metadata.annotations {
            annotations.insert("batch.kubernetes.io/job-completion-index".to_string(), i.to_string());
        }
    ...
    let all_pods: Vec<Pod> = storage.list(&format!("/registry/pods/{}/", namespace))
        .await
        .unwrap();
...
async fn job_with_success_policy_at_least_once_should_succeed_early() {
    ...
    if let Some(conditions) = &status.conditions {
        let complete = conditions.iter().find(|c| 
            c.condition_type == "Complete" && c.status == "True"
        );
        assert!(complete.is_some(), "Job should be Complete when successPolicy is satisfied");
    }
}
...
async fn job_with_managed_by_should_coordinate_with_external_controller() {
    ...
    let pods: Vec<Pod> = storage.list(&format!("/registry/pods/{}/", namespace))
        .await
        .unwrap();
    assert!(!pods.is_empty(), "Pods should be created even with managedBy");
}
...
async fn job_with_node_affinity_should_respect_scheduling_constraints() {
    ...
    let pod = &pods[0];
    assert!(
        pod.spec.affinity.is_some(),
        "Pod should have affinity settings"
    );
    assert!(
        pod.spec.affinity.as_ref().unwrap().node_affinity.is_some(),
...
async fn job_with_pod_failure_policy_fail_job_should_fail_on_exit_code() {
    ...
    if let Some(conditions) = &status.conditions {
        let failed = conditions.iter().find(|c| 
...
async fn job_should_respect_parallelism_across_multiple_reconciles() {
    ...
    let pods: Vec<Pod> = storage.list(&format!("/registry/pods/{}/", namespace))
        .await
        .unwrap();
    assert_eq!(pods.len(), 6, "Should create 3 more pods after first batch succeeds");
    ...
    let pods: Vec<Pod> = storage.list(&format!("/registry/pods/{}/", namespace))
        .await
        .unwrap();
    assert_eq!(pods.len(), 10, "Should create only 1 final pod to reach completions");
}
